...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($action, ['hh_add_member','hh_remove_member','hh_update_member','hh_add_suggested'])) {
        ...
        // ── Add member — now supports picking a registered resident by name/Barangay ID ──
        if ($action === 'hh_add_member') {
            ...
            if ($resId) {
                // A registered resident was picked from the searchable list
                $r = dbFetchOne(
                    "SELECT CONCAT(first_name,' ',last_name) AS full_name
                     FROM residents WHERE id=? AND household_id IS NULL",
                    [$resId]
                );
                if (!$r) jsonError('That resident was not found or already belongs to a household.');
            ...
            if ($resId) {
                dbExecute("UPDATE residents SET household_id=? WHERE id=?", [$hhId, $resId]);
            }
...
// (Not yet in any household, excluding self)
$candidateResidents = [];
if ($household) {
    $candidateResidents = dbFetchAll(
        "SELECT id, CONCAT(first_name,' ',last_name) AS full_name, barangay_id
         FROM residents
         WHERE id != ? AND household_id IS NULL AND status != 'Inactive'
         ORDER BY first_name",
        [$residentId]
    );
}
...
<div id="hhAddModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:1000;align-items:center;justify-content:center">
  <div style="background:white;border-radius:12px;padding:28px;width:90%;max-width:420px">
    ...
    <div style="display:grid;gap:14px">
      ...
      <!-- Searchable resident picker: type a name/Barangay ID and matches appear -->
      <div class="form-group">
        <label>Search Registered Resident <span style="font-weight:400;color:#9ca3af">(optional)</span></label>
...
<script>
// Map "Full Name (BRGY-ID)" -> {id, full_name} for the searchable picker
...
// Fires as the person types in the search box; when the typed text
// exactly matches a datalist option, we auto-fill the name field
// and lock in the resident_id.
function hhAmResidentSearchChanged(value) {
  ...
  if (match) {
    ...
    document.getElementById('hhAmPickedLabel').textContent = '✓ Linked to registered resident: ' + match.full_name;
...
async function hhAddMember() {
  ...
  if (!fullName || !relation) { showToast('Name and relation are required.', 'error'); return; }
